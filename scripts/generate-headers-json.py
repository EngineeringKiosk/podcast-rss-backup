...
def generate_headers_json(response_headers_file: str, rss_url: str, http_code: int) -> dict:
...
def main():
    if len(sys.argv) != 5:
        print(f"Usage: {sys.argv[0]} <response_headers_file> <rss_url> <http_code> <output_file>", file=sys.stderr)
        sys.exit(1)
